...
export default function EditSaleWizard() {
  ...
  const loadSaleData = async () => {
    ...
    try {
      const saleData = await saleDatabase.show(Number(id));
      ...
      // Preencher todos os estados com os dados carregados
      setCustomerId(saleData.customer_id);
      setSaleDate(saleData.sale_date ? new Date(saleData.sale_date) : new Date());
      setNotes(saleData.notes || '');
      setPaymentMethod(saleData.payment_method);
      setInstallments(saleData.installments?.toString() || '1');

      // Formatar discount para display
      if (saleData.discount) {
        setDiscount(formatCurrency((saleData.discount / 100).toString()));
      }
      ...
      // Verificar status e data de pagamento
      if (saleData.status === 'completed' && saleData.payment_date) {
        setMarkAsCompleted(true);
        setPaymentDate(new Date(saleData.payment_date));
      }

      // Definir primeira data de vencimento
      if (saleData.due_date) {
        setFirstDueDate(new Date(saleData.due_date));
      }
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      // Para edição, precisamos remover a venda antiga e criar uma nova
      // pois o método update atual é limitado
      await saleDatabase.remove(Number(id));
            ...
            >
              {isLoading ? (
                ...
                <>
                  ...
                  <Text style={styles.finishButtonText}>Salvar Alterações</Text>
                </>
              )}
            </TouchableOpacity>
          )}
